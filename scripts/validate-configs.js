 ...
 */
async function main() {
  const egiftcardConfigs = getMetamaskConfigs();
  ...
  const prettierViolations = getViolationsMap(egiftcardConfigs);
  const minimalismViolations = getViolationsMap(egiftcardConfigs);
  ...
  await Promise.all(
    Object.entries(egiftcardConfigs).map(
